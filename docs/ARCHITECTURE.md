...
MRI-LMICs-survey/
├── data/
│   ├── data-clean.csv          # REVISED: 51 primary studies (validated)
│   └── fleiss_kappa_matrix.csv # Calibration set for 11 reviewers
├── docs/
│   ├── ARCHITECTURE.md         # Current file
│   └── STATISTICAL_METHODS.md  # Scientific methodology
├── scripts/
│   ├── analysis/
...
1.  **Normalization**: All statistical modules import `utils.py` to ensure architectures, field strengths, and metrics are normalized identically across the repo.
...
- **Cross-Validation**: Random Forest uses Leave-One-Out (LOO) CV for the small dataset (n=51).
